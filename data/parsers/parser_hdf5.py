...
class HDF5Parser:
    ...
    def parse_file(self, file_path: str) -> Tuple[Dict[str, Tuple[np.ndarray, np.ndarray]], Dict[str, Any]]:
        ...
        result_signals = {}
        metadata = {"source_file": file_path, "parser": "HDF5Parser"}

        for name, values in signals.items():
            result_signals[name] = (timestamps, values)

        Logger.log_message_static(f"Parser-HDF5: Successfully parsed {len(signals)} signals from HDF5 file", Logger.INFO)
    ...
    def _extract_data_from_hdf5(self, h5_file) -> Tuple[np.ndarray, Dict[str, np.ndarray]]:
                ...
                try:
                    ...
                    if timestamps is None and self._could_be_timestamps(data, name):
                        ...
                        if timestamps is not None:
                            Logger.log_message_static(f"Parser-HDF5: Using dataset '{name}' as timestamps", Logger.DEBUG)
                            return
                    ...
                    if processed_signal is not None:
                        ...
                        Logger.log_message_static(f"Parser-HDF5: Added signal '{name}' with {len(processed_signal)} points", Logger.DEBUG)

                except Exception as e:
                    Logger.log_message_static(f"Parser-HDF5: Error processing dataset '{name}': {str(e)}", Logger.WARNING)
        ...
        if timestamps is None and signals:
            Logger.log_message_static("Parser-HDF5: No timestamps found, generating synthetic timestamps", Logger.WARNING)
    ...
    def _process_timestamp_data(self, data: np.ndarray, name: str) -> np.ndarray:
        ...
        try:
            ...
            Logger.log_message_static(f"Parser-HDF5: Error processing timestamps from '{name}': {str(e)}", Logger.WARNING)
            return None
        ...
        try:
            ...
            if data.ndim > 1:
                ...
                if data.shape[1] == 1:
                    ...
                    Logger.log_message_static(f"Parser-HDF5: Dataset '{name}' is multidimensional, taking first column", Logger.WARNING)
                    data = data[:, 0]
            ...
            if data.dtype == bool:
                ...
                if np.any(np.isin(upper_data, ['TRUE', 'FALSE'])):
                    Logger.log_message_static(f"Parser-HDF5: Detected boolean strings in dataset '{name}'", Logger.DEBUG)
                    return upper_data
                ...
                try:
                    ...
                    Logger.log_message_static(f"Parser-HDF5: Successfully converted string dataset '{name}' to numeric", Logger.DEBUG)
                    return numeric_data.astype(np.float32)
                except (ValueError, TypeError):
                    Logger.log_message_static(f"Parser-HDF5: Dataset '{name}' contains non-numeric strings, skipping", Logger.WARNING)
                    return None

            else:
                Logger.log_message_static(f"Parser-HDF5: Unsupported data type in dataset '{name}': {data.dtype}", Logger.WARNING)
                return None

        except Exception as e:
            Logger.log_message_static(f"Parser-HDF5: Error processing signal data for '{name}': {str(e)}", Logger.WARNING)
            return None
    ...
    def _align_data_lengths(self, timestamps: np.ndarray, signals: Dict[str, np.ndarray]) -> Tuple[np.ndarray, Dict[str, np.ndarray]]:
        """
        Ensure all signals have the same length as timestamps.
        ...
        for signal_name, signal_data in signals.items():
            if len(signal_data) == target_length:
                ...
                # Truncate signal
                Logger.log_message_static(f"Parser-HDF5: Truncating signal '{signal_name}' from {len(signal_data)} to {target_length} points", Logger.WARNING)
                aligned_signals[signal_name] = signal_data[:target_length]
            else:
                # Signal too short - pad with last value or NaN
                Logger.log_message_static(f"Parser-HDF5: Signal '{signal_name}' too short ({len(signal_data)} < {target_length}), padding", Logger.WARNING)
                if signal_data.dtype.kind in ['U', 'S']:  # String data
                    padded = np.pad(signal_data, (0, target_length - len(signal_data)), mode='edge')
                else:  # Numeric data
                    padded = np.pad(signal_data, (0, target_length - len(signal_data)), mode='constant', constant_values=np.nan)
