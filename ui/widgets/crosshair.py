...
class Crosshair:
    ...
    def __init__(self, viewbox: pg.ViewBox):
        ...
        """
        Logger.log_message_static("Widget-Crosshair: Initializing crosshair overlay", Logger.DEBUG)
        self.viewbox = viewbox
        self.enabled = False

        # Dashed cross lines
        Logger.log_message_static("Widget-Crosshair: Creating vertical and horizontal infinite lines", Logger.DEBUG)
        ...
        # Labels
        Logger.log_message_static("Widget-Crosshair: Creating text labels for coordinates", Logger.DEBUG)
        ...
        if isinstance(scene, pg.GraphicsScene):
            Logger.log_message_static("Widget-Crosshair: Setting up mouse movement signal proxy with rate limiting", Logger.DEBUG)
            ...
            Logger.log_message_static("Widget-Crosshair: Warning: ViewBox scene is not a GraphicsScene, mouse tracking may not work", Logger.WARNING)

        self.mouse_locked = False
        self.local_tz = datetime.now().astimezone().tzinfo
        Logger.log_message_static(f"Widget-Crosshair: Using local timezone: {self.local_tz}", Logger.DEBUG)

        Logger.log_message_static("Widget-Crosshair: Initially hiding crosshair elements", Logger.DEBUG)
        self.hide()
        Logger.log_message_static("Widget-Crosshair: Crosshair initialization complete", Logger.DEBUG)
    ...
    def toggle(self, state: bool):
        ...
        """
        Logger.log_message_static(f"Widget-Crosshair: Toggling crosshair to {'enabled' if state else 'disabled'}", Logger.INFO)
        self.enabled = state
        self.viewbox.setMouseEnabled(x=not state, y=not state)
        if state:
            Logger.log_message_static("Widget-Crosshair: Disabling mouse zoom/pan for crosshair mode", Logger.DEBUG)
            # workaround to prevent zooming on first activation
            QTimer.singleShot(0, lambda: self.viewbox.setMouseEnabled(x=False, y=False))
            self.show()
        else:
            Logger.log_message_static("Widget-Crosshair: Re-enabling mouse zoom/pan", Logger.DEBUG)
            self.hide()

    def show(self):
        """Show crosshair lines and labels."""
        Logger.log_message_static("Widget-Crosshair: Showing crosshair elements", Logger.DEBUG)
        self.vline.show()
        self.hline.show()
        self.label_x.show()
    ...
    def hide(self):
        ...
        Logger.log_message_static("Widget-Crosshair: Hiding crosshair elements", Logger.DEBUG)
        self.vline.hide()
        self.hline.hide()
        self.label_x.hide()
    ...
    def mouse_moved(self, evt):
        ...
        try:
            ...
            Logger.log_message_static(f"Widget-Crosshair: Crosshair position: x={x:.3f}, y={y:.3f}", Logger.DEBUG)
            ...
            try:
                ...
                Logger.log_message_static(f"Widget-Crosshair: Formatted timestamp: {time_str}", Logger.DEBUG)
            except Exception as e:
                Logger.log_message_static(f"Widget-Crosshair: Failed to format timestamp from value {x}: {str(e)}", Logger.WARNING)
            ...
            Logger.log_message_static(f"Widget-Crosshair: Error updating crosshair: {str(e)}", Logger.ERROR)