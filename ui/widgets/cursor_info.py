...
class CursorInfoDialog(QDialog):
    ...
    def __init__(self, parent=None, viewer=None):
        Logger.log_message_static("Widget-Cursor: Initializing CursorInfoDialog", Logger.DEBUG)
        ...
        Logger.log_message_static("Widget-Cursor: Creating cursor info dialog UI components", Logger.DEBUG)
        ...
        Logger.log_message_static("Widget-Cursor: CursorInfoDialog initialization complete", Logger.DEBUG)
    ...
    def toggle_format_mode(self):
        ...
        Logger.log_message_static(f"Widget-Cursor: Toggling to {'scientific' if self._scientific_mode else 'fixed-point'} notation", Logger.INFO)
        ...
        self._rebuild_table()
        Logger.log_message_static("Widget-Cursor: Table rebuilt with new number format", Logger.DEBUG)
    ...
    def update_data(self, time_a: str, time_b: str, values_a: dict, values_b: dict, has_a: bool, has_b: bool, date: str):
        ...
        """
        Logger.log_message_static(f"Widget-Cursor: Updating cursor data: A={'active' if has_a else 'inactive'}, B={'active' if has_b else 'inactive'}", Logger.DEBUG)
        Logger.log_message_static(f"Widget-Cursor: Cursor A time: {time_a}, Cursor B time: {time_b}", Logger.DEBUG)
        Logger.log_message_static(f"Widget-Cursor: Values A contains {len(values_a)} signals, Values B contains {len(values_b)} signals", Logger.DEBUG)

        if has_a and has_b:
            delta_t = self._calc_delta_seconds(time_a, time_b)
            delta_t_str = self.calc_time_delta(time_a, time_b)
            Logger.log_message_static(f"Widget-Cursor: Time delta calculated: {delta_t_str} ({delta_t} seconds)", Logger.DEBUG)
        else:
            delta_t = None
            delta_t_str = "-"
            Logger.log_message_static("Widget-Cursor: Cannot calculate time delta, missing cursor", Logger.DEBUG)
        ...
        Logger.log_message_static(f"Widget-Cursor: Processing {len(keys)} unique signals from both cursors", Logger.DEBUG)
        ...
        for key in keys:
            ...
            Logger.log_message_static(f"Widget-Cursor: Processing signal '{clean_name}' [{unit}], boolean: {is_bool}", Logger.DEBUG)
            Logger.log_message_static(f"Widget-Cursor:   A value: {a_val}, B value: {b_val}", Logger.DEBUG)

            if is_bool:
                Logger.log_message_static(f"Widget-Cursor: Boolean signal '{clean_name}', delta/dps not calculated", Logger.DEBUG)
                ...
                try:
                    ...
                    if a_val and b_val and a_val != "" and b_val != "":
                        ...
                        Logger.log_message_static(f"Widget-Cursor:   Delta calculated: {delta}", Logger.DEBUG)

                        # Calculate delta per second if time delta available
                        if delta_t is not None and delta_t > 0:
                            dps = delta / delta_t
                            Logger.log_message_static(f"Widget-Cursor:   Delta per second: {dps}", Logger.DEBUG)
                        else:
                            dps = None
                            Logger.log_message_static("Widget-Cursor:   Cannot calculate delta per second, invalid time delta", Logger.DEBUG)
                    else:
                        delta = None
                        dps = None
                        Logger.log_message_static("Widget-Cursor:   Cannot calculate delta, missing value at cursor A or B", Logger.DEBUG)
                except (ValueError, TypeError) as e:
                    # Handle conversion errors gracefully
                    Logger.log_message_static(f"Widget-Cursor:   Error calculating delta: {str(e)}", Logger.WARNING)
                    delta = None
                    dps = None
        ...
        Logger.log_message_static(f"Widget-Cursor: Processed {len(self._current_table_data)} signals for cursor info display", Logger.DEBUG)
        self._rebuild_table()
    ...
    def export_to_csv(self):
        ...
        """
        Logger.log_message_static("Widget-Cursor: Opening file dialog for CSV export", Logger.INFO)
        fname, _ = QFileDialog.getSaveFileName(self, "Save CSV", "cursor_values.csv", "CSV files (*.csv)")

        if fname:
            Logger.log_message_static(f"Widget-Cursor: Exporting to CSV file: {fname}", Logger.INFO)
            try:
                with open(fname, 'w', newline='', encoding='utf-8') as f:
                    ...
                    for row in self._export_data[1:]:
                        writer.writerow(row)
                Logger.log_message_static(f"Widget-Cursor: Successfully exported {len(self._export_data)} rows to CSV", Logger.INFO)
            except Exception as e:
                Logger.log_message_static(f"Widget-Cursor: Error exporting to CSV: {str(e)}", Logger.ERROR)
        else:
            Logger.log_message_static("Widget-Cursor: CSV export canceled by user", Logger.DEBUG)

    def showEvent(self, event):
        """
        Ensures dialog window doesn't reset position on show.
        """
        Logger.log_message_static("Widget-Cursor: Showing cursor info dialog", Logger.DEBUG)
        self.move(self.pos())
    ...
    def _is_boolean(self, signal_name: str) -> bool:
        ...
        Logger.log_message_static(f"Widget-Cursor: Cannot determine if {signal_name} is boolean: missing signal_axis_map",
                                  Logger.WARNING)
        return False
    ...
    def _format_val(self, val, unit="", is_bool=False):
        ...
        if val is None or str(val).strip() == "":
            Logger.log_message_static("Widget-Cursor: Formatting empty/None value", Logger.DEBUG)
            return "-"

        if is_bool:
            Logger.log_message_static(f"Widget-Cursor: Formatting boolean value: {val}", Logger.DEBUG)
            return val  # Boolean values are already formatted as "On"/"Off"

        try:
            num_val = float(str(val).replace(',', '.'))
            Logger.log_message_static(f"Widget-Cursor: Converting value {val} to float: {num_val}", Logger.DEBUG)
        except (ValueError, TypeError) as e:
            Logger.log_message_static(f"Widget-Cursor: Error converting value to float: {str(e)}", Logger.WARNING)
            return str(val)

        if self._scientific_mode:
            Logger.log_message_static(f"Widget-Cursor: Formatting {num_val} in scientific notation", Logger.DEBUG)
            formatted = f"{num_val:.6e}"
        else:
            Logger.log_message_static(f"Widget-Cursor: Formatting {num_val} in fixed-point notation", Logger.DEBUG)
        ...
        Logger.log_message_static(f"Widget-Cursor: Final formatted value: {result}", Logger.DEBUG)
        return result
    ...
    def _rebuild_table(self):
        ...
        """
        Logger.log_message_static(f"Widget-Cursor: Rebuilding table with {len(self._current_table_data)} rows", Logger.DEBUG)
        ...
        for i, row in enumerate(self._current_table_data):
            Logger.log_message_static(f"Widget-Cursor: Formatting row {i}: {row['name']}", Logger.DEBUG)
            ...
            if row["is_bool"]:
                Logger.log_message_static(f"Widget-Cursor: Boolean signal, using '-' for delta values", Logger.DEBUG)
        ...
        Logger.log_message_static("Widget-Cursor: Table rebuild complete", Logger.DEBUG)
    ...
    def extract_unit(signal_name: str) -> str:
        ...
        if match:
            ...
            Logger.log_message_static(f"Widget-Cursor: Extracted unit '{unit}' from signal '{signal_name}'", Logger.DEBUG)
            return "" if unit == "-" else unit

        Logger.log_message_static(f"Widget-Cursor: No unit found in signal name '{signal_name}'", Logger.DEBUG)
        return ""
    ...
    def clean_signal_name(signal_name: str) -> str:
        ...
        Logger.log_message_static(f"Widget-Cursor: Cleaned signal name from '{signal_name}' to '{cleaned}'", Logger.DEBUG)
        return cleaned
    ...
    def calc_time_delta(t1_str: str, t2_str: str) -> str:
        ...
        """
        Logger.log_message_static(f"Widget-Cursor: Calculating time delta between {t1_str} and {t2_str}", Logger.DEBUG)
        try:
            fmt = "%H:%M:%S.%f"
            ...
            Logger.log_message_static(f"Widget-Cursor: Time delta calculated: {result}", Logger.DEBUG)
            return result
        except Exception as e:
            Logger.log_message_static(f"Widget-Cursor: Error calculating time delta: {str(e)}", Logger.WARNING)
            return "-"
    ...
    def _calc_delta_seconds(t1_str, t2_str) -> float | None:
        ...
        """
        Logger.log_message_static(f"Widget-Cursor: Calculating seconds delta between {t1_str} and {t2_str}", Logger.DEBUG)
        try:
            fmt = "%H:%M:%S.%f"
            t1 = datetime.datetime.strptime(t1_str, fmt)
            t2 = datetime.datetime.strptime(t2_str, fmt)
            delta_seconds = abs((t2 - t1).total_seconds())
            Logger.log_message_static(f"Widget-Cursor: Delta seconds calculated: {delta_seconds}", Logger.DEBUG)
            return delta_seconds
        except Exception as e:
            Logger.log_message_static(f"Widget-Cursor: Error calculating delta seconds: {str(e)}", Logger.WARNING)
            return None