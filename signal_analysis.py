...
class SignalAnalysisDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        """
        SignalViewer.log_message_static("Initializing SignalAnalysisDialog", INFO)
        ...
        self._plot_windows = []
        SignalViewer.log_message_static("Creating UI for signal analysis dialog", DEBUG)
        self.setup_ui()
        self.update_signal_list()
        SignalViewer.log_message_static("SignalAnalysisDialog initialization complete", DEBUG)

    def setup_ui(self):
        """
        Create and arrange the user interface components for the dialog.
        """
        SignalViewer.log_message_static("Setting up UI components for signal analysis dialog", DEBUG)
        ...
        # ===== Basic Analysis Tab =====
        SignalViewer.log_message_static("Creating Basic Analysis tab", DEBUG)
        basic_tab = QWidget()
        ...
        SignalViewer.log_message_static("Creating Advanced Analysis tab", DEBUG)
        adv_tab = QWidget()
        ...
        # ===== Cross Analysis Tab =====
        SignalViewer.log_message_static("Creating Cross Analysis tab", DEBUG)
        cross_tab = QWidget()
        ...
        SignalViewer.log_message_static("Creating Explanations tab", DEBUG)
        ...
        SignalViewer.log_message_static("UI setup complete for signal analysis dialog", DEBUG)

    def create_signal_selector(self):
        """Create a signal selection dropdown."""
        SignalViewer.log_message_static("Creating signal selector dropdown", DEBUG)
    ...
    def update_signal_list(self):
        ...
        """
        SignalViewer.log_message_static("Updating signal lists in all dropdowns", DEBUG)
        if hasattr(self.parent, 'data_signals'):
            signals = list(self.parent.data_signals.keys())
            SignalViewer.log_message_static(f"Found {len(signals)} available signals", DEBUG)
            ...
            SignalViewer.log_message_static("No data_signals attribute found in parent", WARNING)
    ...
    def get_selected_signal(self, combo=None):
        ...
        if not signal:
            SignalViewer.log_message_static("No signal selected in dropdown", WARNING)
            return None

        SignalViewer.log_message_static(f"Selected signal: '{signal}'", DEBUG)
        return signal
    ...
    def clear_results(self):
        ...
        """
        SignalViewer.log_message_static("Clearing results area", DEBUG)
    ...
    def show_statistics(self):
        ...
        SignalViewer.log_message_static("Calculating basic statistics", INFO)
        signal = self.get_selected_signal()
        if not signal:
            SignalViewer.log_message_static("Cannot calculate statistics: No signal selected", WARNING)
            return

        SignalViewer.log_message_static(f"Computing statistics for signal '{signal}'", DEBUG)
        try:
            # Get the signal values directly
            _, values = self.parent.data_signals[signal]
            SignalViewer.log_message_static(f"Retrieved {len(values)} data points for statistics", DEBUG)
            ...
            SignalViewer.log_message_static("Successfully calculated basic statistics", DEBUG)
            self.show_analysis_results("Statistics", signal, stats)

        except KeyError:
            SignalViewer.log_message_static(f"Signal '{signal}' not found in data_signals", ERROR)
        except Exception as e:
            SignalViewer.log_message_static(f"Error calculating statistics: {str(e)}", ERROR)

    def show_fft(self):
        """Perform FFT analysis on the selected signal and display in a new window."""
        SignalViewer.log_message_static("Preparing FFT analysis", INFO)
        signal = self.get_selected_signal()
        if not signal:
            SignalViewer.log_message_static("Cannot perform FFT: No signal selected", WARNING)
            return

        try:
            # Get the source signal data
            time_arr, values = self.parent.data_signals[signal]
            SignalViewer.log_message_static(f"Retrieved {len(values)} data points for FFT analysis", DEBUG)

            # Process the signal values directly
            SignalViewer.log_message_static("Preparing signal for FFT processing", DEBUG)
            processed_values = prepare_signal_for_analysis(self, values, "FFT Input Signal")
            if processed_values is None:
                SignalViewer.log_message_static("FFT analysis canceled by user", INFO)
                return  # User canceled the operation

            # Perform FFT analysis
            SignalViewer.log_message_static("Calculating sampling frequency", DEBUG)
            if len(time_arr) < 2:
                SignalViewer.log_message_static("Time array has insufficient points for frequency calculation", WARNING)
                fs = 1.0  # Default value
            else:
                fs = 1 / np.mean(np.diff(time_arr))  # Sampling frequency

            SignalViewer.log_message_static(f"Calculated sampling frequency: {fs} Hz", DEBUG)
            n = len(processed_values)

            SignalViewer.log_message_static("Performing FFT calculation", DEBUG)
            fft_values = np.fft.rfft(processed_values)
            freqs = np.fft.rfftfreq(n, d=1 / fs)
            magnitudes = np.abs(fft_values) / n * 2  # Scale appropriately
            SignalViewer.log_message_static(f"FFT calculation complete, {len(freqs)} frequency points generated", DEBUG)

            # Create a proper window using QMainWindow
            SignalViewer.log_message_static("Creating FFT plot window", DEBUG)
            ...
            # Show the window
            plot_window.show()
            SignalViewer.log_message_static("FFT plot window displayed successfully", INFO)
            ...
            SignalViewer.log_message_static(f"Error in FFT analysis: {str(e)}", ERROR)

    def show_time_analysis(self):
        """Perform time-domain analysis on the selected signal and display results."""
        SignalViewer.log_message_static("Performing time-domain analysis", INFO)
        signal = self.get_selected_signal()
        if not signal:
            SignalViewer.log_message_static("Cannot perform time analysis: No signal selected", WARNING)
            return

        try:
            # Get the source signal data directly
            time_arr, values = self.parent.data_signals[signal]
            SignalViewer.log_message_static(f"Retrieved {len(values)} data points for time analysis", DEBUG)

            # Time domain analysis
            if len(time_arr) < 2:
                SignalViewer.log_message_static("Time array has insufficient points", WARNING)
                duration = 0
                sample_rate = 0
            else:
            ...
            SignalViewer.log_message_static(f"Signal duration: {duration}s, Sample rate: {sample_rate}Hz", DEBUG)

            # Calculate zero crossings
            SignalViewer.log_message_static("Calculating zero crossings", DEBUG)
            zero_crossings = np.sum(np.diff(np.signbit(values).astype(int)) != 0)

            # Calculate signal energy and power
            SignalViewer.log_message_static("Calculating signal energy and power", DEBUG)
            ...
            SignalViewer.log_message_static("Time-domain analysis complete", DEBUG)
            self.show_analysis_results("Time Analysis", signal, analysis)

        except KeyError:
            SignalViewer.log_message_static(f"Signal '{signal}' not found in data_signals", ERROR)
        except Exception as e:
            SignalViewer.log_message_static(f"Error in time-domain analysis: {str(e)}", ERROR)

    def show_psd_analysis(self):
        """Calculate and display Power Spectral Density for the selected signal."""
        SignalViewer.log_message_static("Preparing PSD analysis", INFO)
        signal = self.get_selected_signal(self.adv_signal_combo)
        if not signal:
            SignalViewer.log_message_static("Cannot perform PSD analysis: No signal selected", WARNING)
            return

        try:
            # Get the source signal data
            time_arr, values = self.parent.data_signals[signal]
            SignalViewer.log_message_static(f"Retrieved {len(values)} data points for PSD analysis", DEBUG)

            # Process the signal values directly
            SignalViewer.log_message_static("Preparing signal for PSD processing", DEBUG)
            processed_values = prepare_signal_for_analysis(self, values, "PSD Input Signal")
            if processed_values is None:
                SignalViewer.log_message_static("PSD analysis canceled by user", INFO)
                return  # User canceled the operation

            # Calculate PSD
            SignalViewer.log_message_static("Calculating sampling frequency for PSD", DEBUG)
            fs = 1 / np.mean(np.diff(time_arr)) if len(time_arr) > 1 else 1  # Sampling frequency
            SignalViewer.log_message_static(f"Using sampling frequency of {fs} Hz for PSD calculation", DEBUG)

            SignalViewer.log_message_static("Computing Welch PSD with nperseg=256", DEBUG)
            freqs, psd = sc_signal.welch(processed_values, fs=fs, nperseg=256)
            SignalViewer.log_message_static(f"PSD calculation complete, {len(freqs)} frequency bins generated", DEBUG)
            ...
            SignalViewer.log_message_static(f"PSD peak frequency: {peak_freq} Hz, max power: {max_power_db} dB", DEBUG)
            ...
            # Create a proper window using QMainWindow
            SignalViewer.log_message_static("Creating PSD plot window", DEBUG)
            plot_window = QMainWindow(self)
            ...
            SignalViewer.log_message_static("Converting PSD to dB for visualization", DEBUG)
            psd_db = 10 * np.log10(psd)
            p.plot(freqs, psd_db, pen='g')

            # Add reference line at peak frequency
            SignalViewer.log_message_static(f"Adding peak frequency marker at {peak_freq} Hz", DEBUG)
            ...
            # Show the window
            plot_window.show()
            SignalViewer.log_message_static("PSD plot window displayed successfully", INFO)
            ...
            SignalViewer.log_message_static(f"Error in PSD analysis: {str(e)}", ERROR)

    def show_autocorrelation(self):
        """Calculate and display the autocorrelation of the selected signal."""
        SignalViewer.log_message_static("Preparing autocorrelation analysis", INFO)
        signal = self.get_selected_signal(self.adv_signal_combo)
        if not signal:
            SignalViewer.log_message_static("Cannot perform autocorrelation: No signal selected", WARNING)
            return

        try:
            # Get the source signal data
            time_arr, values = self.parent.data_signals[signal]
            SignalViewer.log_message_static(f"Retrieved {len(values)} data points for autocorrelation", DEBUG)

            # Process the signal values
            SignalViewer.log_message_static("Preparing signal for autocorrelation", DEBUG)
            processed_values = prepare_signal_for_analysis(self, values, "Autocorrelation Input Signal")
            if processed_values is None:
                SignalViewer.log_message_static("Autocorrelation analysis canceled by user", INFO)
                return  # User canceled the operation

            # Calculate autocorrelation
            SignalViewer.log_message_static("Computing autocorrelation", DEBUG)
            autocorr = correlate(processed_values, processed_values, mode='full')

            # Normalize
            SignalViewer.log_message_static("Normalizing autocorrelation", DEBUG)
            autocorr = autocorr / np.max(autocorr)

            # Keep only the second half (positive lags)
            center = len(autocorr) // 2
            autocorr = autocorr[center:]
            SignalViewer.log_message_static(f"Extracted {len(autocorr)} positive lag points", DEBUG)
            ...
            if len(time_arr) > 1:
                ...
                lags = lags * dt
                SignalViewer.log_message_static(f"Using time step of {dt}s for lag calculation", DEBUG)

            # Create a proper window
            SignalViewer.log_message_static("Creating autocorrelation plot window", DEBUG)
            plot_window = QMainWindow(self)
            ...
            # Show the window
            plot_window.show()
            SignalViewer.log_message_static("Autocorrelation plot window displayed successfully", INFO)
            ...
            SignalViewer.log_message_static(f"Error in autocorrelation analysis: {str(e)}", ERROR)
    ...
    def show_peak_detection(self):
        ...
        SignalViewer.log_message_static("Preparing peak detection analysis", INFO)
        signal = self.get_selected_signal(self.adv_signal_combo)
        if not signal:
            SignalViewer.log_message_static("Cannot perform peak detection: No signal selected", WARNING)
            return

        try:
            # Get the source signal data directly
            time_arr, values = self.parent.data_signals[signal]
            SignalViewer.log_message_static(f"Retrieved {len(values)} data points for peak detection", DEBUG)

            # Determine if we should look for positive or negative peaks
            signal_mean = np.mean(values)
            SignalViewer.log_message_static(f"Signal mean value: {signal_mean}", DEBUG)
            ...
            if np.all(values < 0) or (
                ...
                SignalViewer.log_message_static("Signal is predominantly negative, inverting to detect valleys", DEBUG)
                processed_values = -values
                peak_type = "negative"
            else:
                # For positive or mixed signals, look for positive peaks
                SignalViewer.log_message_static("Signal is predominantly positive or mixed, detecting peaks", DEBUG)
                processed_values = values
                peak_type = "positive"

            # Use default parameters for peak detection
            SignalViewer.log_message_static("Finding peaks with default parameters", DEBUG)
            peaks, properties = sc_signal.find_peaks(processed_values, height=None, distance=None)
            SignalViewer.log_message_static(f"Found {len(peaks)} peaks in signal", DEBUG)

            if len(peaks) == 0:
                SignalViewer.log_message_static("No peaks detected in signal", WARNING)
            ...
            SignalViewer.log_message_static("Calculating peak heights", DEBUG)
            peak_heights = processed_values[peaks]
            if peak_type == "negative":
                # Convert heights back to negative for display
                SignalViewer.log_message_static("Converting heights back to negative values", DEBUG)
                peak_heights = -peak_heights

            SignalViewer.log_message_static("Calculating peak widths", DEBUG)
            ...
            SignalViewer.log_message_static(f"Peak detection summary: {len(peaks)} peaks, type={peak_type}", DEBUG)

            # Create a proper window for visualization
            SignalViewer.log_message_static("Creating peak detection plot window", DEBUG)
            plot_window = QMainWindow(self)
            ...
            SignalViewer.log_message_static("Plotting original signal", DEBUG)
            p.plot(time_arr, values, pen='b')

            # Add peak markers
            SignalViewer.log_message_static("Adding peak markers to plot", DEBUG)
            ...
            # Show the window
            plot_window.show()
            SignalViewer.log_message_static("Peak detection plot window displayed successfully", INFO)

            # Keep reference to prevent garbage collection
            self._plot_windows.append(plot_window)

            # Format peak data for display
            SignalViewer.log_message_static("Formatting peak data for display", DEBUG)
            display_data = {}
            ...
            SignalViewer.log_message_static("Peak detection analysis complete", INFO)

        except KeyError:
            SignalViewer.log_message_static(f"Signal '{signal}' not found in data_signals", ERROR)
        except Exception as e:
            SignalViewer.log_message_static(f"Error in peak detection: {str(e)}", ERROR)
            import traceback
            SignalViewer.log_message_static(f"Peak detection traceback: {traceback.format_exc()}", DEBUG)

    def show_filter_dialog(self):
        """Show dialog for filtering signal and display the filtered result."""
        SignalViewer.log_message_static("Opening filter dialog", INFO)
        signal = self.get_selected_signal(self.adv_signal_combo)
        if not signal:
            SignalViewer.log_message_static("Cannot show filter dialog: No signal selected", WARNING)
            return

        try:
            SignalViewer.log_message_static(f"Initializing filter dialog for signal '{signal}'", DEBUG)
            ...
            SignalViewer.log_message_static("Added filter type selector with options", DEBUG)
            ...
            SignalViewer.log_message_static("Added cutoff frequency control", DEBUG)
            ...
            def update_cutoff_visibility():
                SignalViewer.log_message_static(f"Filter type changed to: {filter_type.currentText()}", DEBUG)
            ...
            # Show the dialog
            SignalViewer.log_message_static("Displaying filter dialog", DEBUG)
            if not filter_dialog.exec():
                SignalViewer.log_message_static("Filter dialog canceled by user", INFO)
                return  # User canceled

            # Get filter parameters
            SignalViewer.log_message_static("User accepted filter dialog, retrieving filter parameters", DEBUG)
            filter_params = {
            ...
            SignalViewer.log_message_static(f"Filter parameters: {filter_params}", DEBUG)

            # Get signal data
            time_arr, values = self.parent.data_signals[signal]
            SignalViewer.log_message_static(f"Retrieved {len(values)} data points for filtering", DEBUG)

            # Calculate sample rate
            if len(time_arr) < 2:
                SignalViewer.log_message_static("Signal too short to determine sample rate", WARNING)
                return
            fs = 1 / np.mean(np.diff(time_arr))
            SignalViewer.log_message_static(f"Calculated sample rate: {fs} Hz", DEBUG)

            # Design the filter
            SignalViewer.log_message_static("Designing Butterworth filter", DEBUG)
            if filter_params["type"] in ["bandpass", "bandstop"]:
                ...
                if Wn[0] >= Wn[1]:
                    SignalViewer.log_message_static("Lower cutoff must be less than upper cutoff", WARNING)
                    return
                SignalViewer.log_message_static(f"Using normalized cutoff frequencies: {Wn}", DEBUG)
            else:
                # For lowpass and highpass, single cutoff frequency
                Wn = filter_params["cutoff"] / (fs / 2)
                SignalViewer.log_message_static(f"Using normalized cutoff frequency: {Wn}", DEBUG)

            try:
                b, a = butter(filter_params["order"], Wn, btype=filter_params["type"])
                SignalViewer.log_message_static("Filter design successful", DEBUG)
            except Exception as e:
                SignalViewer.log_message_static(f"Error designing filter: {str(e)}", ERROR)
                return

            # Apply the filter
            SignalViewer.log_message_static("Applying filter to signal", DEBUG)
            try:
                filtered_values = filtfilt(b, a, values)
                SignalViewer.log_message_static("Filter application successful", DEBUG)
            except Exception as e:
                SignalViewer.log_message_static(f"Error applying filter: {str(e)}", ERROR)
                return

            # Create a proper window for visualization
            SignalViewer.log_message_static("Creating filter result plot window", DEBUG)
            plot_window = QMainWindow(self)
            ...
            # Show the window
            plot_window.show()
            SignalViewer.log_message_static("Filter result plot window displayed successfully", INFO)
            ...
            SignalViewer.log_message_static(f"Error in filter dialog: {str(e)}", ERROR)
            import traceback
            SignalViewer.log_message_static(f"Filter dialog traceback: {traceback.format_exc()}", DEBUG)

    def show_hilbert_transform(self):
        """Perform Hilbert transform to extract amplitude, phase, and frequency information."""
        SignalViewer.log_message_static("Preparing Hilbert transform analysis", INFO)
        signal = self.get_selected_signal(self.adv_signal_combo)
        if not signal:
            SignalViewer.log_message_static("Cannot perform Hilbert transform: No signal selected", WARNING)
            return

        try:
            # Get the source signal data
            time_arr, values = self.parent.data_signals[signal]
            SignalViewer.log_message_static(f"Retrieved {len(values)} data points for Hilbert transform", DEBUG)

            # Process the signal values
            SignalViewer.log_message_static("Preparing signal for Hilbert transform", DEBUG)
            processed_values = prepare_signal_for_analysis(self, values, "Hilbert Transform Input Signal")
            if processed_values is None:
                SignalViewer.log_message_static("Hilbert transform canceled by user", INFO)
                return  # User canceled the operation

            # Perform Hilbert transform
            SignalViewer.log_message_static("Computing Hilbert transform", DEBUG)
            analytic_signal = hilbert(processed_values)

            # Extract attributes
            SignalViewer.log_message_static("Extracting amplitude envelope", DEBUG)
            amplitude_envelope = np.abs(analytic_signal)

            SignalViewer.log_message_static("Extracting instantaneous phase", DEBUG)
            ...
            SignalViewer.log_message_static("Calculating sample rate for instantaneous frequency", DEBUG)
            fs = 1 / np.mean(np.diff(time_arr))
            SignalViewer.log_message_static(f"Sample rate: {fs} Hz", DEBUG)

            SignalViewer.log_message_static("Computing instantaneous frequency", DEBUG)
            ...
            # Create a proper window
            SignalViewer.log_message_static("Creating Hilbert transform plot window", DEBUG)
            plot_window = QMainWindow(self)
            ...
            # Show the window
            plot_window.show()
            SignalViewer.log_message_static("Hilbert transform plot window displayed successfully", INFO)

            # Keep reference to prevent garbage collection
            self._plot_windows.append(plot_window)

            # Calculate some summary statistics
            SignalViewer.log_message_static("Calculating summary statistics for Hilbert transform", DEBUG)
            hilbert_stats = {
            ...
            SignalViewer.log_message_static("Hilbert transform analysis complete", INFO)

        except KeyError:
            SignalViewer.log_message_static(f"Signal '{signal}' not found in data_signals", ERROR)
        except Exception as e:
            SignalViewer.log_message_static(f"Error in Hilbert transform: {str(e)}", ERROR)
            import traceback
            SignalViewer.log_message_static(f"Hilbert transform traceback: {traceback.format_exc()}", DEBUG)

    def show_energy_analysis(self):
        """Analyze and visualize the energy distribution of the signal."""
        SignalViewer.log_message_static("Preparing energy analysis", INFO)
        signal = self.get_selected_signal(self.adv_signal_combo)
        if not signal:
            SignalViewer.log_message_static("Cannot perform energy analysis: No signal selected", WARNING)
            return

        try:
            # Get the source signal data
            time_arr, values = self.parent.data_signals[signal]
            SignalViewer.log_message_static(f"Retrieved {len(values)} data points for energy analysis", DEBUG)

            # Process the signal values
            SignalViewer.log_message_static("Preparing signal for energy analysis", DEBUG)
            processed_values = prepare_signal_for_analysis(self, values, "Energy Analysis Input Signal")
            if processed_values is None:
                SignalViewer.log_message_static("Energy analysis canceled by user", INFO)
                return  # User canceled the operation
            ...
            SignalViewer.log_message_static("Calculating sample rate", DEBUG)
            fs = 1 / np.mean(np.diff(time_arr))
            SignalViewer.log_message_static(f"Sample rate: {fs} Hz", DEBUG)

            # Calculate energy in time domain
            SignalViewer.log_message_static("Computing time domain energy", DEBUG)
            energy_time = np.sum(np.abs(processed_values) ** 2)

            # Calculate energy density in frequency domain
            SignalViewer.log_message_static("Computing FFT for frequency domain energy", DEBUG)
            n = len(processed_values)
            fft_values = np.fft.rfft(processed_values)
            freqs = np.fft.rfftfreq(n, d=1 / fs)
            ...
            SignalViewer.log_message_static("Computing cumulative energy", DEBUG)
            ...
            SignalViewer.log_message_static("Finding energy threshold frequencies", DEBUG)
            for threshold in energy_thresholds:
                idx = np.where(cumulative_energy >= threshold)[0]
                freq = freqs[idx[0]] if len(idx) > 0 else freqs[-1]
                threshold_freqs.append(freq)
                SignalViewer.log_message_static(f"{threshold * 100}% energy frequency threshold: {freq} Hz", DEBUG)

            # Create a proper window
            SignalViewer.log_message_static("Creating energy analysis plot window", DEBUG)
            plot_window = QMainWindow(self)
            ...
            SignalViewer.log_message_static("Adding threshold markers to cumulative energy plot", DEBUG)
            ...
            # Show the window
            plot_window.show()
            SignalViewer.log_message_static("Energy analysis plot window displayed successfully", INFO)

            # Keep reference to prevent garbage collection
            self._plot_windows.append(plot_window)

            # Calculate some summary statistics
            SignalViewer.log_message_static("Calculating summary statistics for energy analysis", DEBUG)
            energy_stats = {
            ...
            SignalViewer.log_message_static("Energy analysis complete", INFO)

        except KeyError:
            SignalViewer.log_message_static(f"Signal '{signal}' not found in data_signals", ERROR)
        except Exception as e:
            SignalViewer.log_message_static(f"Error in energy analysis: {str(e)}", ERROR)
            import traceback
            SignalViewer.log_message_static(f"Energy analysis traceback: {traceback.format_exc()}", DEBUG)

    def show_phase_analysis(self):
        """Analyze and display phase information of the signal."""
        SignalViewer.log_message_static("Preparing phase analysis", INFO)
        signal = self.get_selected_signal(self.adv_signal_combo)
        if not signal:
            SignalViewer.log_message_static("Cannot perform phase analysis: No signal selected", WARNING)
            return

        try:
            # Get the source signal data
            time_arr, values = self.parent.data_signals[signal]
            SignalViewer.log_message_static(f"Retrieved {len(values)} data points for phase analysis", DEBUG)

            # Process the signal values directly
            SignalViewer.log_message_static("Preparing signal for phase analysis", DEBUG)
            processed_values = prepare_signal_for_analysis(self, values, "Phase Analysis Input Signal")
            if processed_values is None:
                SignalViewer.log_message_static("Phase analysis canceled by user", INFO)
                return  # User canceled the operation

            # Calculate Hilbert transform to get phase information
            SignalViewer.log_message_static("Computing Hilbert transform for phase extraction", DEBUG)
            analytic_signal = hilbert(processed_values)

            # Extract instantaneous phase and unwrap to avoid phase jumps
            SignalViewer.log_message_static("Extracting and unwrapping instantaneous phase", DEBUG)
            phase = np.unwrap(np.angle(analytic_signal))

            # Calculate phase statistics
            SignalViewer.log_message_static("Computing phase statistics", DEBUG)
            phase_stats = {
            ...
            SignalViewer.log_message_static(f"Phase statistics calculated: {phase_stats}", DEBUG)

            # Create a proper window using QMainWindow
            SignalViewer.log_message_static("Creating phase analysis plot window", DEBUG)
            plot_window = QMainWindow(self)
            ...
            SignalViewer.log_message_static("Computing phase velocity for secondary plot", DEBUG)
            ...
            # Show the window
            plot_window.show()
            SignalViewer.log_message_static("Phase analysis plot window displayed successfully", INFO)

            # Keep reference to prevent garbage collection
            self._plot_windows.append(plot_window)

            # Display results in the dialog
            SignalViewer.log_message_static("Displaying phase analysis results in main dialog", DEBUG)
            self.show_analysis_results("Phase Analysis", signal, phase_stats)
            SignalViewer.log_message_static("Phase analysis complete", INFO)

        except KeyError:
            SignalViewer.log_message_static(f"Signal '{signal}' not found in data_signals", ERROR)
        except Exception as e:
            SignalViewer.log_message_static(f"Error in phase analysis: {str(e)}", ERROR)
            import traceback
            SignalViewer.log_message_static(f"Phase analysis traceback: {traceback.format_exc()}", DEBUG)

    def show_cepstrum_analysis(self):
        """Analyze and display cepstrum analysis of the signal."""
        SignalViewer.log_message_static("Preparing cepstrum analysis", INFO)
        signal = self.get_selected_signal(self.adv_signal_combo)
        if not signal:
            SignalViewer.log_message_static("Cannot perform cepstrum analysis: No signal selected", WARNING)
            return

        try:
            # Get the source signal data
            time_arr, values = self.parent.data_signals[signal]
            SignalViewer.log_message_static(f"Retrieved {len(values)} data points for cepstrum analysis", DEBUG)

            # Process the signal values
            SignalViewer.log_message_static("Preparing signal for cepstrum analysis", DEBUG)
            processed_values = prepare_signal_for_analysis(self, values, "Cepstrum Analysis Input Signal")
            if processed_values is None:
                SignalViewer.log_message_static("Cepstrum analysis canceled by user", INFO)
                return  # User canceled the operation

            # Calculate sample rate
            if len(time_arr) < 2:
                SignalViewer.log_message_static("Signal too short for cepstrum analysis", WARNING)
                return

            fs = 1 / np.mean(np.diff(time_arr))
            SignalViewer.log_message_static(f"Sample rate calculated: {fs} Hz", DEBUG)

            # Compute cepstrum
            SignalViewer.log_message_static("Computing FFT for cepstrum analysis", DEBUG)
            n = len(processed_values)
            fft_values = np.fft.fft(processed_values)

            # Logarithm of the power spectrum
            SignalViewer.log_message_static("Computing log power spectrum", DEBUG)
            ...
            SignalViewer.log_message_static("Computing inverse FFT to obtain cepstrum", DEBUG)
            cepstrum = np.fft.ifft(log_power).real

            # Create quefrency array (time-like x-axis for cepstrum)
            quefrency = np.arange(n) / fs
            SignalViewer.log_message_static(f"Created quefrency array with {len(quefrency)} points", DEBUG)

            # Create a proper window
            SignalViewer.log_message_static("Creating cepstrum analysis plot window", DEBUG)
            plot_window = QMainWindow(self)
            ...
            SignalViewer.log_message_static("Plotted log power spectrum (positive frequencies)", DEBUG)
            ...
            SignalViewer.log_message_static("Plotted cepstrum (first half)", DEBUG)

            # Detect peaks in the cepstrum that might indicate periodicity
            SignalViewer.log_message_static("Detecting peaks in cepstrum", DEBUG)
            ...
            if skip_points >= n_half:
                ...
                SignalViewer.log_message_static(f"Adjusted skip points to {skip_points} (10% of data)", WARNING)
            ...
            if len(peaks) > 0:
                SignalViewer.log_message_static(f"Found {len(peaks)} peaks in cepstrum", DEBUG)
                # Add peak markers
                ...
                SignalViewer.log_message_static("No significant peaks found in cepstrum", WARNING)
            ...
            # Show the window
            plot_window.show()
            SignalViewer.log_message_static("Cepstrum analysis plot window displayed successfully", INFO)

            # Keep reference to prevent garbage collection
            self._plot_windows.append(plot_window)

            # Prepare results
            SignalViewer.log_message_static("Preparing cepstrum analysis results", DEBUG)
            cepstrum_stats = {
            ...
            # Display results in the dialog
            SignalViewer.log_message_static("Displaying cepstrum analysis results in main dialog", DEBUG)
            self.show_analysis_results("Cepstrum Analysis", signal, cepstrum_stats)
            SignalViewer.log_message_static("Cepstrum analysis complete", INFO)

        except KeyError:
            SignalViewer.log_message_static(f"Signal '{signal}' not found in data_signals", ERROR)
        except Exception as e:
            SignalViewer.log_message_static(f"Error in cepstrum analysis: {str(e)}", ERROR)
            import traceback
            SignalViewer.log_message_static(f"Cepstrum analysis traceback: {traceback.format_exc()}", DEBUG)

    def show_wavelet_dialog(self):
        """Show dialog for wavelet analysis and display the results."""
        SignalViewer.log_message_static("Opening wavelet analysis dialog", INFO)
        signal = self.get_selected_signal(self.adv_signal_combo)
        if not signal:
            SignalViewer.log_message_static("Cannot perform wavelet analysis: No signal selected", WARNING)
            return

        try:
            # Get the source signal data
            time_arr, values = self.parent.data_signals[signal]
            SignalViewer.log_message_static(f"Retrieved {len(values)} data points for wavelet analysis", DEBUG)

            # Process the signal values
            SignalViewer.log_message_static("Preparing signal for wavelet analysis", DEBUG)
            processed_values = prepare_signal_for_analysis(self, values, "Wavelet Analysis Input Signal")
            if processed_values is None:
                SignalViewer.log_message_static("Wavelet analysis canceled by user", INFO)
                return  # User canceled the operation

            # Create wavelet settings in a dialog
            SignalViewer.log_message_static("Initializing wavelet dialog settings", DEBUG)
            ...
            SignalViewer.log_message_static(f"Added wavelet type selector with {len(available_wavelets)} options",
                                            DEBUG)
            ...
            SignalViewer.log_message_static("Added scales spinbox with range 1-128", DEBUG)
            ...
            # Show the dialog
            SignalViewer.log_message_static("Displaying wavelet dialog", DEBUG)
            if not wavelet_dialog.exec():
                SignalViewer.log_message_static("Wavelet dialog canceled by user", INFO)
                return

            # Get parameters
            SignalViewer.log_message_static("User accepted wavelet dialog, retrieving parameters", DEBUG)
            ...
            SignalViewer.log_message_static(
                f"Wavelet parameters: type={wavelet_name}, scales={num_scales}, log_scale={is_log_scale}", DEBUG)

            # Calculate sample rate
            if len(time_arr) < 2:
                SignalViewer.log_message_static("Signal too short for wavelet analysis", WARNING)
                return
            ...
            # Generate scales
            if is_log_scale:
                SignalViewer.log_message_static("Generating logarithmic scales", DEBUG)
                scales = np.logspace(0, np.log10(num_scales), num_scales)
            else:
                SignalViewer.log_message_static("Generating linear scales", DEBUG)
                scales = np.linspace(1, num_scales, num_scales)

            # Perform wavelet transform
            SignalViewer.log_message_static(f"Performing wavelet transform with {wavelet_name} wavelet", DEBUG)
            try:
                if wavelet_name in ['morl', 'cmor', 'mexh']:
                    # Continuous wavelet transform for these wavelet types
                    SignalViewer.log_message_static("Using continuous wavelet transform (CWT)", DEBUG)
                    ...
                    SignalViewer.log_message_static(f"CWT completed with {len(frequencies)} frequency bands", DEBUG)
                else:
                    # Discrete wavelet transform with multi-level decomposition
                    SignalViewer.log_message_static("Using discrete wavelet transform (DWT)", DEBUG)
                    ...
                    SignalViewer.log_message_static(f"Using DWT at level {level} (max possible: {max_level})", DEBUG)

                    # Perform multilevel DWT
                    coeffs = pywt.wavedec(processed_values, wavelet_name, level=level)
                    SignalViewer.log_message_static(f"DWT completed with {len(coeffs)} coefficient sets", DEBUG)
                    ...
                    SignalViewer.log_message_static(
                        f"Prepared DWT coefficients for display with frequencies {frequencies}", DEBUG)
            except Exception as e:
                SignalViewer.log_message_static(f"Error computing wavelet transform: {str(e)}", ERROR)
                # Consider simpler option if the first attempt fails
                if wavelet_name not in ['morl', 'cmor', 'mexh']:
                    try:
                        SignalViewer.log_message_static("Falling back to simpler wavelet transform", WARNING)
                        wavelet_name = 'morl'
                        coefficients, frequencies = pywt.cwt(processed_values, scales, wavelet_name)
                        frequencies = pywt.scale2frequency(wavelet_name, scales) * fs
                        SignalViewer.log_message_static("Fallback wavelet transform successful", INFO)
                    except Exception as e2:
                        SignalViewer.log_message_static(f"Fallback wavelet transform also failed: {str(e2)}", ERROR)
                        raise Exception(f"Failed to compute wavelet transform: {str(e)}")

            # Create a proper window
            SignalViewer.log_message_static("Creating wavelet analysis plot window", DEBUG)
            plot_window = QMainWindow(self)
            ...
            p1.setLabel('bottom', 'Time (s)')
            p1.plot(time_arr, processed_values, pen='b')
            SignalViewer.log_message_static("Added original signal plot", DEBUG)
            ...
            SignalViewer.log_message_static(f"Coefficient matrix shape: {abs_coeffs.shape}", DEBUG)
            ...
            SignalViewer.log_message_static(f"Set scalogram levels: [{min_val}, {max_val * 0.8}]", DEBUG)
            ...
            if len(frequencies) > 1:
                ...
                SignalViewer.log_message_static(f"Set {len(freq_ticks)} frequency axis ticks", DEBUG)
            ...
            SignalViewer.log_message_static("Added power by frequency plot", DEBUG)
            ...
            # Show the window
            plot_window.show()
            SignalViewer.log_message_static("Wavelet analysis plot window displayed successfully", INFO)

            # Keep reference to prevent garbage collection
            self._plot_windows.append(plot_window)

            # Calculate summary statistics for wavelet analysis
            SignalViewer.log_message_static("Calculating wavelet analysis statistics", DEBUG)
            ...
            SignalViewer.log_message_static(f"Identified dominant frequency: {dominant_freq} Hz", DEBUG)
            ...
            if len(frequencies) > 2:
                ...
                SignalViewer.log_message_static(f"High/low frequency energy ratio: {energy_ratio}", DEBUG)
            else:
                energy_ratio = 1.0
            ...
            # Display results in the dialog
            SignalViewer.log_message_static("Displaying wavelet analysis results in main dialog", DEBUG)
            self.show_analysis_results("Wavelet Analysis", signal, wavelet_stats)
            SignalViewer.log_message_static("Wavelet analysis complete", INFO)

        except KeyError:
            SignalViewer.log_message_static(f"Signal '{signal}' not found in data_signals", ERROR)
        except Exception as e:
            SignalViewer.log_message_static(f"Error in wavelet analysis: {str(e)}", ERROR)
            import traceback
            SignalViewer.log_message_static(f"Wavelet analysis traceback: {traceback.format_exc()}", DEBUG)

            def show_cross_correlation(self):
                """Calculate and display cross-correlation between two signals."""
                SignalViewer.log_message_static("Preparing cross-correlation analysis", INFO)
                ...
                if not signal1 or not signal2:
                    SignalViewer.log_message_static(
                        "Cannot perform cross-correlation: One or both signals not selected", WARNING)
                    return

                try:
                    SignalViewer.log_message_static(f"Computing cross-correlation between '{signal1}' and '{signal2}'",
                                                    DEBUG)
                    # Get the source signal data directly
                    ...
                    SignalViewer.log_message_static(
                        f"Signal lengths - {signal1}: {len(values1)}, {signal2}: {len(values2)}", DEBUG)

                    # Handle signals with different lengths by padding the shorter one
                    if len(values1) > len(values2):
                        SignalViewer.log_message_static(f"Padding {signal2} to match length of {signal1}", DEBUG)
                        values2 = np.pad(values2, (0, len(values1) - len(values2)), 'constant')
                    elif len(values2) > len(values1):
                        SignalViewer.log_message_static(f"Padding {signal1} to match length of {signal2}", DEBUG)
                        values1 = np.pad(values1, (0, len(values2) - len(values1)), 'constant')

                    # Calculate cross-correlation
                    SignalViewer.log_message_static("Computing cross-correlation with full mode", DEBUG)
                    corr = correlate(values1, values2, mode='full')

                    # Normalize to [-1, 1] range
                    SignalViewer.log_message_static("Normalizing cross-correlation values", DEBUG)
                    norm_factor = np.sqrt(np.sum(values1 ** 2) * np.sum(values2 ** 2))
                    if norm_factor > 0:
                        corr_normalized = corr / norm_factor
                    else:
                        SignalViewer.log_message_static("Warning: Zero normalization factor in cross-correlation",
                                                        WARNING)
                        corr_normalized = corr

                    # Create lag time array
                    SignalViewer.log_message_static("Creating lag time axis", DEBUG)
                    sample_rate1 = 1.0 / np.mean(np.diff(time_arr1))
                    lags = np.arange(-len(corr_normalized) // 2 + 1, len(corr_normalized) // 2 + 1) / sample_rate1

                    # Create a proper window
                    SignalViewer.log_message_static("Creating cross-correlation plot window", DEBUG)
                    plot_window = QMainWindow(self)
                    ...
                    SignalViewer.log_message_static("Created cross-correlation plot", DEBUG)
                    ...
                    SignalViewer.log_message_static(f"Maximum correlation: {max_corr:.4f} at lag {max_lag:.4f}s", DEBUG)
                    ...
                    SignalViewer.log_message_static("Added original signals comparison plot", DEBUG)
                    ...
                    if abs(max_lag) > 0.001:  # Only if lag is significant
                        SignalViewer.log_message_static(f"Adding time-shifted version of {signal2} by {max_lag:.4f}s",
                                                        DEBUG)
                        ...
                        if np.any(valid_mask):
                            ...
                            SignalViewer.log_message_static("Added time-shifted signal to comparison plot", DEBUG)
                        else:
                            SignalViewer.log_message_static("Shifted signal has no overlap with original time range",
                                                            WARNING)
                    ...
                    # Show the window
                    plot_window.show()
                    SignalViewer.log_message_static("Cross-correlation plot window displayed successfully", INFO)

                    # Keep reference to prevent garbage collection
                    self._plot_windows.append(plot_window)

                    # Calculate statistics
                    SignalViewer.log_message_static("Calculating cross-correlation statistics", DEBUG)
                    ...
                    for threshold in threshold_corrs:
                        ...
                        if len(above_threshold) > 0:
                            ...
                            SignalViewer.log_message_static(
                                f"Width at {threshold * 100:.0f}% of max correlation: {lag_range:.4f}s", DEBUG)
                        else:
                            threshold_lags[threshold] = None
                            SignalViewer.log_message_static(f"No points above {threshold * 100:.0f}% threshold found",
                                                            WARNING)

                    # Compute correlation at zero lag
                    zero_lag_idx = len(corr_normalized) // 2
                    zero_lag_correlation = corr_normalized[zero_lag_idx]
                    SignalViewer.log_message_static(f"Correlation at zero lag: {zero_lag_correlation:.4f}", DEBUG)
                    ...
                    # Display results in the dialog
                    SignalViewer.log_message_static("Displaying cross-correlation results in main dialog", DEBUG)
                    self.show_analysis_results("Cross-Correlation Analysis", f"{signal1} & {signal2}", cross_corr_stats)
                    SignalViewer.log_message_static("Cross-correlation analysis complete", INFO)

                except KeyError as ke:
                    SignalViewer.log_message_static(f"Signal not found in data_signals: {str(ke)}", ERROR)
                except Exception as e:
                    SignalViewer.log_message_static(f"Error in cross-correlation analysis: {str(e)}", ERROR)
                    import traceback
                    SignalViewer.log_message_static(f"Cross-correlation analysis traceback: {traceback.format_exc()}",
                                                    DEBUG)

            def show_analysis_results(self, title, signal_name, data_dict):
                """Display analysis results in a table in the results area."""
                SignalViewer.log_message_static(f"Displaying {title} results for {signal_name}", DEBUG)
                self.clear_results()
                ...
                SignalViewer.log_message_static(f"Added results title: {title} for {signal_name}", DEBUG)
                ...
                SignalViewer.log_message_static(f"Creating results table with {len(data_dict)} rows", DEBUG)

                # Populate table
                for i, (key, value) in enumerate(data_dict.items()):
                    table.setItem(i, 0, QTableWidgetItem(key))
                    table.setItem(i, 1, QTableWidgetItem(str(value)))
                    SignalViewer.log_message_static(f"Added table row: {key} = {value}", DEBUG)

                self.results_layout.addWidget(table)
                SignalViewer.log_message_static("Analysis results displayed successfully", DEBUG)
        ...
        class ExplanationTab(QWidget):
            ...
            def __init__(self, parent=None):
                ...
                """
                SignalViewer.log_message_static("Initializing ExplanationTab", DEBUG)
                super().__init__(parent)
                ...
                SignalViewer.log_message_static("Creating explanation sections", DEBUG)
         ...
         - Useful for signal alignment, time delay estimation, and pattern matching
                ...
                SignalViewer.log_message_static("ExplanationTab initialization complete", DEBUG)
            ...
            def add_explanation_group(self, layout, title, text):
                ...
                """
                SignalViewer.log_message_static(f"Adding explanation group: {title}", DEBUG)
                ...
                SignalViewer.log_message_static(f"Added explanation group for: {title}", DEBUG)
        ...
        def prepare_signal_for_analysis(dialog, values, title="Signal Processing"):
            ...
            """
            SignalViewer.log_message_static(f"Preparing signal for {title}", DEBUG)
            try:
                ...
                if np.any(~np.isfinite(values)):
                    ...
                    SignalViewer.log_message_static(
                        f"Signal contains {bad_values} non-finite values ({percent_bad:.2f}%)", WARNING)

                    # Offer options to the user
                    if bad_values / total_values > 0.5:  # More than 50% bad values
                        SignalViewer.log_message_static(
                            "More than 50% of values are non-finite, analysis may not be meaningful", WARNING)
                    ...
                    SignalViewer.log_message_static("Prompting user for handling non-finite values", INFO)
                    msg.exec()

                    if msg.clickedButton() == cancel_button:
                        SignalViewer.log_message_static("User canceled analysis due to non-finite values", INFO)
                        return None
                    elif msg.clickedButton() == replace_button:
                        SignalViewer.log_message_static("Replacing non-finite values with zeros", DEBUG)
                        # Create a copy to avoid modifying the original
                        values_copy = np.copy(values)
                        values_copy[~np.isfinite(values_copy)] = 0.0
                        return values_copy
                    elif msg.clickedButton() == interpolate_button:
                        SignalViewer.log_message_static("Interpolating non-finite values", DEBUG)
                        # Create mask of valid values
                        mask = np.isfinite(values)
                        if not np.any(mask):
                            SignalViewer.log_message_static("No valid values to interpolate from, returning zeros",
                                                            WARNING)
                        ...
                        SignalViewer.log_message_static(f"Interpolated {bad_values} values", DEBUG)
                        return values_copy

                # If we reach here, the signal is already good
                SignalViewer.log_message_static("Signal is ready for analysis (all values are finite)", DEBUG)
                return values
