...
def calculate_harmonic_analysis(signal_data, fundamental_frequency, num_harmonics=10):
    """
    Calculate harmonic analysis for vibration data.

    Args:
        signal_data: The time domain signal data
        fundamental_frequency: The fundamental frequency to analyze harmonics around
        num_harmonics: Number of harmonics to analyze (default: 10)

    Returns:
        dict: Dictionary containing harmonic amplitudes, phases, and frequencies
    """
    import numpy as np
    ...
    # Calculate FFT
    n = len(signal_data)
    fft_result = np.fft.fft(signal_data) / n
    ...
    for i in range(1, num_harmonics + 1):
        ...
        # Store results
        results['frequencies'].append(freqs[freq_idx])
        results['amplitudes'].append(2 * np.abs(fft_result[freq_idx]))
        results['phases'].append(np.angle(fft_result[freq_idx], deg=True))
        results['harmonic_numbers'].append(i)
